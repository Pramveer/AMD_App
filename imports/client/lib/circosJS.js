...
circosJS.Layout = function(conf, data) {
  ...
  for (k in ref) {
    ...
    v.offset = (offset==undefined?0:offset);
    offset += (v.len==undefined?0:v.len);
  }
  this.size = offset;
  gap = this.conf.gap;
  size = this.size;
...
circosJS.renderLayoutLabels = function(conf, d3, block) {
  ...
  label = block.append('text').attr('id', function(d) {
	return 'arc_label_text_' + d.id;
  }).style('font-size', '' + conf.labels.size + 'px').attr('text-anchor', 'middle');
...
circosJS.Core.prototype.render = function(ids, removeTracks) {
  ...
  if (translated.empty()) {
    translated = svg.append('g').attr('class', 'all').attr('transform', 'translate(' + (parseInt(this._layout.conf.outerRadius) + parseInt(this.conf.left)) + ',' +(parseInt(this._layout.conf.outerRadius) + parseInt(this.conf.right)) + ')');
  }
  ref1 = this.tracks;
