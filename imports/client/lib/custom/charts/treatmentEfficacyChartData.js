...
renderTreatmentEfficacyChartNew  = function (medicine, data, container, isPatient) {
    ...
    let dataForSelectedWeeks = getPlotBandRangesForViralLoadChartNew(dataPJ,maxvalue);
...
getPlotBandRangesForViralLoadChartNew = (dataPJ,maxvalue) => {
    ...
    if(!($("#divWeekResponse input[type=radio]:checked").val() == 'all' || $("#divWeekResponse input[type=radio]:checked").val() == undefined )){
        ...
        let range = 0;
        range = $("#divWeekResponse input[type=radio]:checked").val() == undefined ? '0-8' : $("#divWeekResponse input[type=radio]:checked").val();
