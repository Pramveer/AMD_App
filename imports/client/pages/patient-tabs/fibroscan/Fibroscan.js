import './fibroscan.html';


let FibroscanLabsData = [];
patientCount = new ReactiveVar(0);
patientRowNo = 1;

Template.Fibroscan.onCreated(function() {
    let self = this;
    ...
    this.autorun(function() {
        ...
        if (PatientID != '') { let params = getCurrentPopulationFilters();
            ...
            Meteor.call('getLabDetails', PatientID, 'FIBROSCAN',params, function(error, result) {
                if (error) {
                    self.loading.set(false);
                } else {
                    FibroscanLabsData = result;
                    self.loading.set(false);
                }
            });
...
Template.Fibroscan.rendered = function() {
            ...
            if (params != null) {
                ...
                Meteor.call('searchPatients', params, (error, results) => {
                    if (error || (results.length < 0)) {
                        ...
                        //console.log(results);
                        $('.patient-row-fibroscan').html(params['rowNo']);
                        ...
                        $('.fibroscan-patientid').html(Math.abs(patientID));
...
Template.Fibroscan.helpers({
    'isLoading': function() {
        return Template.instance().loading.get();
    },
    //check for no data
    'isDataPresent': () => FibroscanLabsData.length > 0 ? true : false,
    ...
    'getPatientLab': function() {
        if (FibroscanLabsData) {
            setTimeout(function() {
                // $('.pin-labs-fibro .table-fibro').bdt({
                //     pageRowCount: 15,
                //     arrowDown: 'fa-angle-down',
                //     arrowUp: 'fa-angle-up'
                // });

                $('.pin-labs-fibro .table-fibro').bootstrapTable();
            }, 200);
            return FibroscanLabsData;
        }
    },
    'PercentToNormal': function(pValue) {
        return pValue && parseFloat(pValue) > 0 ? parseFloat(parseFloat(pValue) / 100).toFixed(3) : 0;
    }
});
