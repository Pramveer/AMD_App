 ...
 * @reviewer: 
 * @date: 29-May-2017
 * @desc: Ued 'setCohortPatientCount' method to set unique patient count on cohort
 *  setCohortPatientCount({patientCount:drugsData[0]['TotalN']});
             
               $("#providerTab-N").html(commaSeperatedNumber(getHTMLTextN(drugsData[0]['TotalN'],drugsData[0]['TotalN'])));
                
*/
Template.Provider.onCreated(function() {
    var self = this;
     //this.ReactivedrugsData = new ReactiveVar([]);
     //this.ReactivedrugRiskData = new ReactiveVar([]);
    ...
    this.autorun(function() {
        ...
        if(localStorage.getItem('provider_relative_weights')){
            ...
            params['costWeight'] = relativeWeights['cost'];
             ...
             //Added Jayesh 15th 2017 for adding reset weight slider for provider tab.
        ...
        Meteor.call('getProviderPageData', params, function(error, result) {
            if (error) {
               ...
               // self.ReactivedrugsData.set(drugsData);
               // self.ReactivedrugRiskData.set(drugRiskData);


                //console.log(drugsData);

                //$('.searchPatientCountHeader').html(commaSeperatedNumber(drugsData[0]['TotalN']));
               setCohortPatientCount({patientCount:drugsData[0]['TotalN']});
             
               $("#providerTab-N").html(commaSeperatedNumber(getHTMLTextN(drugsData[0]['TotalN'],drugsData[0]['TotalN'])));
                //hide div on primary care physician
                if(isPrimaryCarePhysician()){
                  hideProviderClasses();
                }
...
Template.Provider.rendered = function() {
    // added by Yurvaj 1st March
    $('#providerTabMenu').css('display', 'block');
    ...
    initWeightsAccordian();
     // added by Arvind 10th March 2017
    $(".js-pri-decision").click(function(e) {
        //console.log('s-pri-decision Events')
       e.stopPropagation();
  $(".js-provider-weight-panel").slideToggle();
    });
    $(document).click(function(e){
        //console.log('document Events');
        //console.log(e.target);
    if($(".js-provider-weight-panel").css('display')==='block'){
        $(".js-provider-weight-panel").hide();
    }
    });
     $('.provider-relativeWeights-section').click(function(e){
         //console.log('provider-relativeWeights-section');
          //console.log(e.target);
          //Do not stopPropagation event for element(Go button)  with id `js_provider_applyWeights` in that container
          if(e.target.id!=="js_provider_applyWeights"){
        e.stopPropagation();
        }
    });
...
// Template.Provider.destroyed = function () {
//    Session.set('provider_subtab','withFDACompliant');
// };


//function to plot svg bubbles on the chart
Template.Provider.renderSvgChartForSissionData = function(xAxis, yAxis, svgHeight, svgWidth, flag) {
...
Template.Provider.helpers({
    ...
    'DrugInfoUpdated': function() {
        try {
            ...
            //console.log("Rerender DrugInfoUpdated");
            //var DrugsData = Template.instance().ReactivedrugsData.get();
 ...
 // Added by Arvind on 10 March 2017
//If Weightslider not changed then need to hide drugscore column and adjust other columns
 if(!localStorage.isProviderWeightSliderChanges){

    $(".pdl-list>li:nth-child(1)").css('cssText',`width: 130px;
    min-width: 130px;`);
    // Below style is for DrugScore related column
    //  $(".pdl-list>li:nth-child(2)").css('cssText',`width: 130px;
    // min-width: 130px;`);
     $(".pdl-list>li:nth-child(3)").css('cssText',`width: 130px;
    min-width: 130px;`);
     $(".pdl-list>li:nth-child(4)").css('cssText',`width: 320px;
    min-width: 320px;`);
      $(".pdl-list>li:nth-child(5)").css('cssText',`width: 140px;
    min-width: 140px;`);
      $(".pdl-list>li:nth-child(7)").css('cssText',`width: 210px;
    min-width: 210px;`);


 }else{
     //Need to handle if drug score is display which is default
     // As Prioritze Decision Paramater weight changed need to display drugscore column and re adjust for other columns
     $("#drugScoreColumn").show();
$(".pdl-drugscore").show();

    $(".pdl-list>li:nth-child(1)").css('cssText','');
     $(".pdl-list>li:nth-child(3)").css('cssText','');
     $(".pdl-list>li:nth-child(4)").css('cssText','');
      $(".pdl-list>li:nth-child(5)").css('cssText','');
      $(".pdl-list>li:nth-child(7)").css('cssText','');
    ...
    // Added by jayesh 16th May 2017 for if cost is 0, hide cost.
    'isCostVisible': function(cost){
        let tempCost = parseInt(cost);
        return tempCost == null || tempCost == 0 ? false : true;
    },
    'isCostVisibleLabel': function(costMedian, costMode){
         let tempMedian = parseInt(costMedian), tempMode = parseInt(costMode);
        return (tempMedian == null || tempMedian == 0) && (tempMode == null || tempMode == 0) ? false : true;
    },
    ...
    //var DrugRiskData=   Template.instance().ReactivedrugRiskData.get();
...
Template.Provider.events({
    ...
    'click .rweb-list li': function(e, template) {
        ...
        localStorage.setItem('provider_relative_weights', JSON.stringify(relativeWeights));
         //Added Jayesh 15th 2017 for adding reset weight slider for provider tab.
         if((relativeWeights['efficacy'] == 0 && relativeWeights['adherence'] == 0 && relativeWeights['cost'] == 0) ||
         (relativeWeights['efficacy'] == 100 && relativeWeights['adherence'] == 100 && relativeWeights['cost'] == 100))
         {
            localStorage.removeItem("isProviderWeightSliderChanges");
         }
        ...
        // else if (tabData == "preAuthorization") {
    ...
    // Jayesh 02/17/2017 Click event for P & T Model
     ...
     // Arvind 04/03/2017 Click event for CoverMyMeds icon
     'click .js-cmm-epa-btn': function(e) {
    ...
    'click .SortDrugPageBy': function(e) {
        ...
        Router.current().render(Template.Provider);
    ...
    'click div.drugpagination ul > li.numpg': function(event, template) {
        ...
        $('.paginationDrugs .pdl-list').each(function(d) {
         ...
         $('html, body').animate({
            scrollTop: $('.pdl-header .pdl-list').position().top
        }, 'slow');
    },
    ...
    'click div.drugpagination ul > li.pg-next': function(event, template) {
        ...
        // $('li.pg-prev').removeClass('disable');
        // $('li.pg-next').addClass('disable');
        if(!event.currentTarget.className.includes('disable'))
        {
        let pValue = JSON.parse(localStorage.AllDrugsData).length;
        let page_numbers = pValue / showPages;
        if (pValue % showPages == 0) {
    ...
    'change .changeYAxis': function() {
        ...
        // Template.Provider.renderSvgChartForSissionData($('.changeXAxis').val(), $('.changeYAxis').val(), 500, 790);
        //Template.Provider.renderSvgChartForSissionData($('.changeXAxis').val(), $('.changeYAxis').val(), 500, 950);
...
if($(".js-provider-weight-panel").css('display')==='block'){
    ...
    // Added By Arvind 9th March 17
     'change #txtEffWeight_Provider': function()  {

  localStorage.isProviderWeightSliderChanges=true;
     ...
     ,
    // Added By Arvind 9th March 17
     'change #txtCostWeight_Provider': function()  {
           localStorage.isProviderWeightSliderChanges=true;
     ...
     ,
    // Added By Arvind 9th March 17
     'change #txtAdhWeight_Provider': function()  {
           localStorage.isProviderWeightSliderChanges=true;
...
function updateReactiveDrugsDataWithBestScore({data,sliderName}){
        //let params = getCurrentPopulationFilters();
console.log('Update reactive var based on :'+sliderName);
        let relativeWeights = {};
        relativeWeights['efficacy'] = parseFloat($('#txtEffWeight_Provider').val());
        relativeWeights['adherence'] = parseFloat($('#txtAdhWeight_Provider').val());
        relativeWeights['cost'] = parseFloat($('#txtCostWeight_Provider').val());
        ...
        localStorage.setItem('provider_relative_weights', JSON.stringify(relativeWeights));
        //1. Get saved data to temp variable
        //2. Then re calculate best drugg score based on weight
        //3. Update reactive var and re render provider tab

       let tempDrugsData=  data; // Template.instance().ReactivedrugsData.get();
          //console.log("Efficacy slider is clicked");
          //console.log(tempDrugsData);

//Re calculate Drug score based on Updated weight
let cost_factor=0;
          tempDrugsData=_.map(tempDrugsData,function(drug){
              // cost factor need to calculate for each drug reccord also
                 cost_factor = ((drug.Max_cost - drug.Cost.TotalCost) / drug.Max_cost) * 100;
      if (drug.Efficacy.Efficacy == 'NA') {
        drugScore = (relativeWeights.adherence * drug.Adherence.Adherence + relativeWeights.cost * cost_factor) / ((relativeWeights.adherence + relativeWeights.cost) * 10) ;
    } else {
        drugScore = (relativeWeights.adherence * drug.Adherence.Adherence + relativeWeights.efficacy * drug.Efficacy.Efficacy + relativeWeights.cost * cost_factor) /((relativeWeights.adherence + relativeWeights.efficacy + relativeWeights.cost) * 10);
    }
    //Update drugScore field
    // Added by Arvind 10th March 17 .If all weight parameter is zero  then `drugScore` value will be Infinity which produce NaN.
    drugScore = isNaN(drugScore) ? 0 : drugScore;
    drug.drugScore=parseFloat(drugScore).toFixed(2);
return drug;
          });

//Get All drugscore field and find maximum score as Best Drug score
          let allDrugScores = _.pluck(tempDrugsData, "drugScore");
    //  Converted output string value to float as `toFixed()` method return always string
    allDrugScores = allDrugScores.map(function (score) {

        return parseFloat(parseFloat(score).toFixed(2));
    });

    let bestDrugScore = _.max(allDrugScores);
    //Now Update best drugs score field `Best_Drug_Score` in all records
    tempDrugsData = _.map(tempDrugsData, function(drug) {
     return _.extend({}, drug, {     Best_Drug_Score:bestDrugScore
        });
     });
     // Now return updated data variable with updated value and see magic
          return tempDrugsData;
}
...
let isDrugPrescribed = () => {
    ...
    if (localStorage.ProviderID && localStorage.PatientID && localStorage.lastSelectedDrug) {
        ...
        //Show error message if Provider/Patient/Drug is missing
    }
...
Template.Provider.popupLoaded = function(obj) {
...
Template.Provider.helpers({
    ...
    // Added By Arvind 10th March 17
    'isDrugScoreVisible': function(){
//       //  console.log('isDrugScoreVisible');
//          // Based on Weight slider changed drug score column will be displayed, so er are checking relative value slider
//         let drugScoreStyle=localStorage.getItem('isProviderWeightSliderChanges')?"":"display:none";
//        // console.log(drugScoreStyle)
//        if(drugScoreStyle){
//            // If Not Empty then need to set width dynamically for provider content
//        }
// return  drugScoreStyle;
    },
    ...
    'CreateSVGForValueZone': function(DrugsData, DrugName, DrugSequence) {
        setTimeout(function() {
            ...
            //
            // });
//console.log("Is all popover reinitialized");
            // $('.pop').each(function() {
            ...
            //New way to draw chart in Provider
            //commented old Canvas Circle
            //  drawCircle();
            //hide the load mask
...
function renderSvgForValueZone(xAxis, yAxis, data, DrugSequence) {
     ...
     * @reviewer: 
     * @date: 18-May-2017
     * @desc: Commented below condition which restrict drawing chart for adherence value 0
     * So I have commented that code
     * Based on only first record we can't conclude for rest
    */
...
console.log("data-toggle=popover RENDERED:::::*****");
    ...
    $('[data-toggle=popover]').on('click', function(e) {
        ...
        //check for relative value chart icon popup
            ...
            setTimeout(function() {
                ...
                //Commented on 08-DEC-2016 for increasing width of relative chart
                // Template.Provider.renderSvgChartForSissionData('Cost', 'Efficacy', 500, 790);
                //Template.Provider.renderSvgChartForSissionData('Cost', 'Efficacy', 500, 950);
...
function executeAtRender(me) {
    ...
    setCohortHeaderMenu({tabName:"provider"});
...
Template.Provider.getDataForRiskSections = function(obj) {
    var drug = $(obj).attr('drug').trim();
    var sectionName = $(obj).attr('section').trim();
...
function renderNewRelativeValueChart(container, xData, yData, height, width) {
    let dataObj = {
        ...
        axis: { x: xData, y: yData },
        sizeParams: { svgHeight: height, svgWidth: width }
    };
...
Template.Provider.renderNewRelativeValueChart = function(xData, yData, height, width) {
...
// Added By Jayesh 14th March For Sorting By Relative Value
let SortingDrugData = () =>{
    if(localStorage.SortDrugsBy == 'drugScore' || localStorage.SortDrugsBy == 'value' )
    {
        if(localStorage.isProviderWeightSliderChanges){
            localStorage.setItem('SortDrugsBy', 'drugScore');
            if(!localStorage.SortingType)
            localStorage.setItem('SortingType', 'desc');
        } else {
            localStorage.setItem('SortDrugsBy', 'value');
            if(!localStorage.SortingType)
...
//hide data based on primary physician
let hideProviderClasses = ()=>{
  $('.pro-evidance-div').hide();
  $('.btngroup').hide();
  setTimeout(()=>{
      $('.pro-evidance-div').hide();
      $('.btngroup').hide();
